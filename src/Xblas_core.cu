...
{
  hipblasStatus_t status = cublasXgemm( handle->cublas_handle,
                                       transa == KBLAS_Trans ? HIPBLAS_OP_T : HIPBLAS_OP_N,
                                       transb == KBLAS_Trans ? HIPBLAS_OP_T : HIPBLAS_OP_N,
                                       m, n, k,
                                       &alpha, A, lda,
                                               B, ldb,
                                       &beta,  C, ldc);
...
{
  hipblasStatus_t status = cublasXsyrk( handle->cublas_handle,
                                       uplo == KBLAS_Lower ? HIPBLAS_FILL_MODE_LOWER : HIPBLAS_FILL_MODE_UPPER,
                                       trans == KBLAS_Trans ? HIPBLAS_OP_T : HIPBLAS_OP_N,
                                       m, n,
                                       &alpha, A, lda,
                                       &beta,  B, ldb);
  check_error_ret(status, KBLAS_cuBLAS_Error);
...
int kblasXsymm( kblasHandle_t handle,
                ...
                const TYPE beta,        TYPE *C, int ldc)
{
  hipblasStatus_t status = cublasXsymm( handle->cublas_handle,
                                      side == KBLAS_Left ? HIPBLAS_SIDE_LEFT : HIPBLAS_SIDE_RIGHT,
                                      uplo == KBLAS_Lower ? HIPBLAS_FILL_MODE_LOWER : HIPBLAS_FILL_MODE_UPPER,
                                      m, n,
                      ...
                      TYPE* B, int ldb)
{
  ...
  hipblasStatus_t status = kblasXtrsm( handle->cublas_handle,
                                      side == KBLAS_Left ? HIPBLAS_SIDE_LEFT : HIPBLAS_SIDE_RIGHT,
                                      uplo == KBLAS_Lower ? HIPBLAS_FILL_MODE_LOWER : HIPBLAS_FILL_MODE_UPPER,
                                      trans == KBLAS_Trans ? HIPBLAS_OP_T : HIPBLAS_OP_N,
                                      diag == KBLAS_NonUnit ? HIPBLAS_DIAG_NON_UNIT : HIPBLAS_DIAG_UNIT,
                                      m, n,
                                      &alpha, A, lda,
                                              B, ldb);
  check_error_ret(status, KBLAS_cuBLAS_Error);
                      ...
                      TYPE* B, int ldb)
{
  //TODO if cuda version >= 8, call cublas instead
  //TODO verify this is better than cublas if cuda >= 8
  hipblasStatus_t status = kblasXtrmm( handle->cublas_handle,
                                      side == KBLAS_Left ? HIPBLAS_SIDE_LEFT : HIPBLAS_SIDE_RIGHT,
                                      uplo == KBLAS_Lower ? HIPBLAS_FILL_MODE_LOWER : HIPBLAS_FILL_MODE_UPPER,
                                      trans == KBLAS_Trans ? HIPBLAS_OP_T : HIPBLAS_OP_N,
                                      diag == KBLAS_NonUnit ? HIPBLAS_DIAG_NON_UNIT : HIPBLAS_DIAG_UNIT,
