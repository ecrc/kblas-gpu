...
template<class T>
inline void copyGPUArray(T* originalArray, T* copyArray, int num_ptrs, hipStream_t stream)
{
	thrust::copy(
...
inline void ara_trsm_set_offset_dims(
	...
	int num_ops, hipStream_t stream
)
{
	thrust::for_each(
		thrust::cuda::par.on(stream),
...
int kblas_ara_batch_set_samples(
	...
	int samples, int r, int num_ops, hipStream_t stream
)
{
...
)
{
	if(bs == 32)
	{
		...
		hipLaunchKernelGGL(HIP_KERNEL_NAME(ara_fused_potrf_kernel<inType, outType, 32>), dim3(dimGrid), dim3(dimBlock), smem_per_block, stream, 
	...
	else if(bs == 16)
	{
		...
		hipLaunchKernelGGL(HIP_KERNEL_NAME(ara_fused_potrf_kernel<inType, outType, 16>), dim3(dimGrid), dim3(dimBlock), smem_per_block, stream, 
			op_samples, A_batch, lda_batch, R_batch, ldr_batch, diag_R, block_ranks, num_ops
		);
...
inline int kblas_ara_svec_count_batch_template(			
	...
	TY** Y_batch, int* ldy_batch, TY tol, int r, int* small_vectors, 
	int relative, int num_ops, hipStream_t stream
)
{
	int ops_per_block = 4;
	int threads_per_block = ops_per_block * bs;
	...
	if(bs == 16)
		hipLaunchKernelGGL(HIP_KERNEL_NAME(ara_svec_count_kernel<TR, TY, 16>), dim3(dimGrid), dim3(dimBlock), 0, stream, 
			diag_R, op_samples, ranks_batch, max_diag, Y_batch, ldy_batch, 
			tol, r, small_vectors, relative, num_ops
		);
	else if(bs == 32)
		hipLaunchKernelGGL(HIP_KERNEL_NAME(ara_svec_count_kernel<TR, TY, 32>), dim3(dimGrid), dim3(dimBlock), 0, stream, 
			diag_R, op_samples, ranks_batch, max_diag, Y_batch, ldy_batch, 
			tol, r, small_vectors, relative, num_ops
		);
	else 
...
int kblas_ara_16x16_trsm(
	T** B_batch, int* ldb_batch, T** A_batch, int* lda_batch, 
	int* rows_batch, int* cols_batch, int offset, int num_ops, 
	int max_rows, hipStream_t stream
)
{
	...
	hipLaunchKernelGGL(ara_16x16_trsm_kernel, dim3(dimGrid), dim3(dimBlock), smem_per_block, stream,  
...
) 
{
	...
	if(max_n <= 16)
	{
		...
		hipLaunchKernelGGL(HIP_KERNEL_NAME(ara_mp_syrk_batch_kernel<float, double, ept, DIM>), dim3(dimGrid), dim3(dimBlock), 0, kblas_stream, m, n, A, lda, B, ldb, batchCount);
	}
	else
	{
		const int ept = 4;
