...
int batch_pstrf_template(kblasHandle_t handle, int dim, T_ptr M_batch, int ldm, int stride_m, Ti_ptr piv_batch, int stride_piv, int* ranks, int num_ops)
{
	...
	if     (dim <= 32) batch_pstrf_shared_kernel<T, T_ptr, Ti_ptr, 32><<< dimGrid, dimBlock, smem_needed, handle->stream >>>(dim, M_batch, ldm, stride_m, piv_batch, stride_piv, ranks, num_ops);
	else if(dim <= 64) batch_pstrf_shared_kernel<T, T_ptr, Ti_ptr, 64><<< dimGrid, dimBlock, smem_needed, handle->stream >>>(dim, M_batch, ldm, stride_m, piv_batch, stride_piv, ranks, num_ops);
