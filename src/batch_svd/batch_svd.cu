...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Random matrix geenration using hiprand for the RSVD
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<class T>
inline int generateRandomMatrices(T* d_m, int rows, int cols, unsigned int seed, int num_ops, hipStream_t stream = 0);

template<>
inline int generateRandomMatrices(float* d_m, int rows, int cols, unsigned int seed, int num_ops, hipStream_t stream)
{
    ...
    hiprandGenerateNormal(gen, d_m, num_ops * rows * cols, 0, 1);
...
template<>
inline int generateRandomMatrices(double* d_m, int rows, int cols, unsigned int seed, int num_ops, hipStream_t stream)
{
    hiprandGenerator_t gen;
    hiprandCreateGenerator(&gen, HIPRAND_RNG_PSEUDO_DEFAULT);
    hiprandSetPseudoRandomGeneratorSeed(gen, seed);
    ...
    hiprandGenerateNormalDouble(gen, d_m, num_ops * rows * cols, 0, 1);
...
int batchSortSingularValues_small(T_ptr M, int ldm, int stride_m, T_ptr S, int stride_s, int rows, int cols, int num_ops, kblasHandle_t handle)
{
	...
	switch(block_size)
	{
		case  32: hipLaunchKernelGGL(HIP_KERNEL_NAME(batchSortSingularValuesSmallKernel<32, T, T_ptr>), dim3(dimGrid), dim3(dimBlock), smem_needed, handle->stream , M, ldm, stride_m, S, stride_s, rows, cols, num_ops); break;
		case  64: hipLaunchKernelGGL(HIP_KERNEL_NAME(batchSortSingularValuesSmallKernel<64, T, T_ptr>), dim3(dimGrid), dim3(dimBlock), smem_needed, handle->stream , M, ldm, stride_m, S, stride_s, rows, cols, num_ops); break;
...
int batch_svd_small(T_ptr M, int ldm, int stride_m, T_ptr S, int stride_s, int rows, int cols, int normalize_cols, int num_ops, kblasHandle_t handle)
{
	...
	#ifdef HLIB_PROFILING_ENABLED
	handle->tic();
	check_error_ret( hipMalloc(&svd_gflops, num_ops * sizeof(double)), KBLAS_Error_Allocation );
	#endif
	...
	{
		case 1: hipLaunchKernelGGL(HIP_KERNEL_NAME(blockSVDKernel<1, T, T_ptr>), dim3(dimGrid), dim3(dimBlock), smem_needed, handle->stream , M, ldm, stride_m, S, stride_s, rows, cols, normalize_cols, num_ops, svd_gflops); break;
		case 2: hipLaunchKernelGGL(HIP_KERNEL_NAME(blockSVDKernel<2, T, T_ptr>), dim3(dimGrid), dim3(dimBlock), smem_needed, handle->stream , M, ldm, stride_m, S, stride_s, rows, cols, normalize_cols, num_ops, svd_gflops); break;
	...
	check_error_ret( hipFree(svd_gflops), KBLAS_Error_Deallocation );
...
int batchNormalizeColumns(T_ptr M, int ldm, int stride_m, T_ptr S, int stride_s, int rows, int cols, int num_ops, kblasHandle_t handle)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(batchNormalizeColumnsKernel<T, T_ptr>), dim3(dimGrid), dim3(dimBlock), 0, handle->stream , M, ldm, stride_m, S, stride_s, rows, cols, rows_per_thread, cols_per_thread, num_ops);
...
int batchMaxOffdiagonal(T_ptr M, int ldm, int stride_m, int rows, int cols, T* offdiagonal, int num_ops, kblasHandle_t handle)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(batchMaxOffdiagonalKernel<T, T_ptr>), dim3(dimGrid), dim3(dimBlock), smem_needed, handle->stream , M, ldm, stride_m, rows, cols, cols_per_thread, offdiagonal, num_ops);
