                    ...
                    int batchCount)
{
  ...
  hipStream_t curStream = handle->stream;
  int2 dims[] = {
    {TX, 2},//1 warps
    {TX, 4},//2 warps
    {TX, 8} //4 warps
  ...
  if(transB == KBLAS_Trans)
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_gemm_NX_registers_MNmulTX_2d<T, true, TX, 8, 8>), dim3(gridDim), dim3(blockDim), 0, curStream, 
                            m, n, k,
                            alpha, A, lda, strideA,
                                   B, ldb, strideB,
                            beta,  C, ldc, strideC);
  else
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_gemm_NX_registers_MNmulTX_2d<T, false, TX, TX, TX>), dim3(gridDim), dim3(blockDim), 0, curStream, 
                            m, n, k,
                            alpha, A, lda, strideA,
                                   B, ldb, strideB,
                            beta,  C, ldc, strideC);
