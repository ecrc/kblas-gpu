...
#define __XPOTI_BATCH_KERNELS_H__
...
dev_poti_U_L_reg_shared_Nfix(int n, T* A, int lda)
{
  const int TX1 = TX + 1;
  //setup shared memory
...
kernel_poti_U_L_reg_shared_Nfix( const int n, int batchCount,
                                  ...
                                  int* info)
{
  ...
  dev_poti_U_L_reg_shared_Nfix<T, TX>(n, A, lda);
...
template<typename T, int TX>
__device__ inline void
dev_poti_U_L_reg_shared_Nvar(int n, T* A, int lda)
{
  const int TX1 = TX + 1;
  //setup shared memory
  extern __shared__ __align__(sizeof(T)) unsigned char sh_data[];
...
template<typename T, typename T_PTR, bool STRIDED, int TX>
__global__ void  //__launch_bounds__(256)
kernel_poti_U_L_reg_shared_Nvar( const int n, int batchCount,
                                  T_PTR A_array, int A_row_off, int A_col_off, int lda, long strideA,
                                  int* info)
{
  //are we within bounds
  ...
  dev_poti_U_L_reg_shared_Nvar<T, TX>(n, A, lda);

}
//==============================================================================================
#else
  #error "Pre-Kepler architechture is not supported in KBLAS batch POTI"
#endif

#endif //__XPOTI_BATCH_KERNELS_H__
