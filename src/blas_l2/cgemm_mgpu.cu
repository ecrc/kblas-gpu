...
/**
 * @file src/blas_l2/cgemm_mgpu.cu
                		...
                		long *tile)
{
	...
	if(tile_size == -1)
	{
		tile_size = recommend_tile(m, n, k, ngpus, CGEMM_MAX_TILE);
		(*tile) = tile_size;
	}
	...
	hipFloatComplex* gpu_ws[MAX_NGPUS];
	hipFloatComplex* a[MAX_NGPUS];
	hipFloatComplex* b[MAX_NGPUS][2];
	hipFloatComplex* c[MAX_NGPUS][2];
	hipFloatComplex* a_[MAX_NGPUS];
	hipFloatComplex* b_[MAX_NGPUS][2];
	hipFloatComplex* c_[MAX_NGPUS][2];
	...
	// allocate gpu memory
	{
		...
		for(long i = 0; i <  ngpus; i++)
		{
			...
			e = hipMalloc((void**)&gpu_ws[i], mem_space * sizeof(hipFloatComplex));
			if(e != hipSuccess)
			{
	...
	// aux host pointers
	// aux pointers
	hipFloatComplex	*A_[MAX_NGPUS], *B_[MAX_NGPUS], *C_[MAX_NGPUS];
	...
	// Adjust pointers
	{
		...
		for(long i = 0; i < ngpus; i++)
		{
			A_[i] = (hipFloatComplex*)A;
			B_[i] = (hipFloatComplex*)B;
			C_[i] = (hipFloatComplex*)C;
		}
		// device
		for(long i = 0; i < ngpus; i++)
		{
	...
	//main loop
	{
		...
		for(long i = 0; i < total_iterations; i++)
		{
			...
			if(pflag)printf("i = %ld, advance A_\n", i);
			 if(transa == 'n' || transa == 'N') for(long id = 0; id < ngpus_active; id++) {A_[id] = (hipFloatComplex*)A + (i *ngpus + id) * tile_size;}
			else for(long id = 0; id < ngpus_active; id++) { A_[id] = (hipFloatComplex*)A + (i * ngpus + id) * tile_size * lda; }
			...
			for(long j = 0; j < n_ ; j++)
			{
				...
				// Advance B_
				if(transb == 'n' || transb == 'N') for(long id = 0; id < ngpus_active; id++) {B_[id] = (hipFloatComplex*)B + j * tile_size * ldb;}
				else for(long id = 0; id < ngpus_active; id++) {B_[id] = (hipFloatComplex*)B + j * tile_size;}

				// Advance C_
				for(long id = 0; id < ngpus_active; id++)
				{
					//C_[id] = (hipFloatComplex*)C + ( (i *ngpus + id) * tile_size ) + ( j * tile_size * ldc);
					C_[id] = (hipFloatComplex*)C;
					//if(transa == 'n' || transa == 'N')
				...
				if(j == 0)
				{
					for(long id = 0; id < ngpus_active; id++)
					{
						...
						se = cublasSetMatrixAsync(rc[id], cc[id], sizeof(hipFloatComplex), C_[id], ldc, c[id][cselect[id]], tile_size, stream[id][3]);
				...
				for(long id = 0; id < ngpus_active; id ++)
				{
					...
					if(j == 0)
					{
						...
						se = cublasSetMatrixAsync(ra[id], ca[id], sizeof(hipFloatComplex), A_[id], lda, a_[id], tile_size, stream[id][1]);
						char ss[100];
					...
					se = cublasSetMatrixAsync(rb[id], cb[id], sizeof(hipFloatComplex), B_[id], ldb, b_[id][bselect[id]], tile_size, stream[id][2]);
				...
				for(p = 0;  p < k_; p++)
				{
					hipFloatComplex beta_;
					if(p == 0)beta_ = beta; else beta_ = make_one<hipFloatComplex>();
					...
					for(long id = 0; id < ngpus_active; id++)
					{
						...
						if(pflag)printf("\t\t p = %ld, gpu = %ld, invoke cgemm\n", p, id);
						if(pflag)printf("\t\t ------------------------------\n");
						...
						if( p == 0)
						{
							if(j != n_-1)
							{
								...
								hipFloatComplex* Ctmp = C_[id] + tile_size * ldc;
								cselect[id] = 1 - cselect[id];
								// rc[id] is the same, but we need to compute cc
								cc[id] = min(n - (j+1)*tile_size, tile_size);
								if(pflag)printf("\t\t cselect[%ld] = %ld \n", id, cselect[id]);
								hipStreamWaitEvent(stream[id][3], _cout_[id][cselect[id]], 0);
								se = cublasSetMatrixAsync(rc[id], cc[id], sizeof(hipFloatComplex), Ctmp, ldc, c_[id][cselect[id]], tile_size, stream[id][3]);
								char ss[100];
				...
				for(long id = 0; id < ngpus_active; id++)
				{
					...
					se = cublasGetMatrixAsync(rc[id], cc[id], sizeof(hipFloatComplex), c_[id][cselect[id]], tile_size, C_[id], ldc, stream[id][3]);
