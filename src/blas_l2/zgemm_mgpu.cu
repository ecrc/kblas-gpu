                		...
                		long *tile)
{
	...
	cuDoubleComplex* gpu_ws[MAX_NGPUS];
	cuDoubleComplex* a[MAX_NGPUS];
	cuDoubleComplex* b[MAX_NGPUS][2];
	cuDoubleComplex* c[MAX_NGPUS][2];
	cuDoubleComplex* a_[MAX_NGPUS];
	cuDoubleComplex* b_[MAX_NGPUS][2];
	cuDoubleComplex* c_[MAX_NGPUS][2];
	...
	// allocate gpu memory
	{
		...
		for(long i = 0; i <  ngpus; i++)
		{
			...
			e = cudaMalloc((void**)&gpu_ws[i], mem_space * sizeof(cuDoubleComplex));
			if(e != cudaSuccess)
			{
	...
	// aux host pointers
	// aux pointers
	cuDoubleComplex	*A_[MAX_NGPUS], *B_[MAX_NGPUS], *C_[MAX_NGPUS];
	...
	// Adjust pointers
	{
		...
		for(long i = 0; i < ngpus; i++)
		{
			A_[i] = (cuDoubleComplex*)A;
			B_[i] = (cuDoubleComplex*)B;
			C_[i] = (cuDoubleComplex*)C;
		}
		// device
		for(long i = 0; i < ngpus; i++)
		{
	...
	//main loop
	{
		...
		for(long i = 0; i < total_iterations; i++)
		{
			...
			if(pflag)printf("i = %ld, advance A_\n", i);
			 if(transa == 'n' || transa == 'N') for(long id = 0; id < ngpus_active; id++) {A_[id] = (cuDoubleComplex*)A + (i *ngpus + id) * tile_size;}
			else for(long id = 0; id < ngpus_active; id++) { A_[id] = (cuDoubleComplex*)A + (i * ngpus + id) * tile_size * lda; }
			...
			for(long j = 0; j < n_ ; j++)
			{
				...
				// Advance B_
				if(transb == 'n' || transb == 'N') for(long id = 0; id < ngpus_active; id++) {B_[id] = (cuDoubleComplex*)B + j * tile_size * ldb;}
				else for(long id = 0; id < ngpus_active; id++) {B_[id] = (cuDoubleComplex*)B + j * tile_size;}

				// Advance C_
				for(long id = 0; id < ngpus_active; id++)
				{
					//C_[id] = (cuDoubleComplex*)C + ( (i *ngpus + id) * tile_size ) + ( j * tile_size * ldc);
					C_[id] = (cuDoubleComplex*)C;
					//if(transa == 'n' || transa == 'N')
				...
				if(j == 0)
				{
					for(long id = 0; id < ngpus_active; id++)
					{
						...
						se = cublasSetMatrixAsync(rc[id], cc[id], sizeof(cuDoubleComplex), C_[id], ldc, c[id][cselect[id]], tile_size, stream[id][3]);
				...
				for(long id = 0; id < ngpus_active; id ++)
				{
					...
					if(j == 0)
					{
						...
						se = cublasSetMatrixAsync(ra[id], ca[id], sizeof(cuDoubleComplex), A_[id], lda, a_[id], tile_size, stream[id][1]);
						char ss[100];
					...
					se = cublasSetMatrixAsync(rb[id], cb[id], sizeof(cuDoubleComplex), B_[id], ldb, b_[id][bselect[id]], tile_size, stream[id][2]);
				...
				for(p = 0;  p < k_; p++)
				{
					cuDoubleComplex beta_;
					if(p == 0)beta_ = beta; else beta_ = make_one<cuDoubleComplex>();
					...
					for(long id = 0; id < ngpus_active; id++)
					{
						...
						if( p == 0)
						{
							if(j != n_-1)
							{
								...
								cuDoubleComplex* Ctmp = C_[id] + tile_size * ldc;
								cselect[id] = 1 - cselect[id];
								// rc[id] is the same, but we need to compute cc
								cc[id] = min(n - (j+1)*tile_size, tile_size);
								if(pflag)printf("\t\t cselect[%ld] = %ld \n", id, cselect[id]);
								cudaStreamWaitEvent(stream[id][3], _cout_[id][cselect[id]], 0);
								se = cublasSetMatrixAsync(rc[id], cc[id], sizeof(cuDoubleComplex), Ctmp, ldc, c_[id][cselect[id]], tile_size, stream[id][3]);
								char ss[100];
				...
				for(long id = 0; id < ngpus_active; id++)
				{
					...
					se = cublasGetMatrixAsync(rc[id], cc[id], sizeof(cuDoubleComplex), c_[id][cselect[id]], tile_size, C_[id], ldc, stream[id][3]);
