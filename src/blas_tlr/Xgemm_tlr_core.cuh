...
int Xgemm_LR_core( kblasHandle_t handle,
                    ...
                    T* d_workspace)
{
  hipStream_t hip_stream[3];
  hip_stream[0] = handle->stream;
  hip_stream[1] = handle->stream;//TODO
  hip_stream[2] = handle->stream;//TODO
  ...
  if(clone_CUV){
    ...
    check_error_ret( hipMemcpy2DAsync ((void*)_Cu, (size_t)(_ldCu * sizeof(T)),
                                        (const void*)Cu, (size_t)(ldCu * sizeof(T)),
                                        (size_t)(M * sizeof(T)), (size_t)(kC),
                                        hipMemcpyDeviceToDevice,
                                        hip_stream[0]), KBLAS_CUDA_Error);

    check_error_ret( hipMemcpy2DAsync ((void*)_Cv, (size_t)(_ldCv * sizeof(T)),
                                        (const void*)Cv, (size_t)(ldCv * sizeof(T)),
                                        (size_t)(N * sizeof(T)), (size_t)(kC),
                                        hipMemcpyDeviceToDevice,
                                        hip_stream[1]), KBLAS_CUDA_Error);
  }

  //----------------------------
  handle->SetStream(hip_stream[1]);{
    // P = AvtBu = Av^T * Bu
  ...
  {
    //concat _Cu+Av
    handle->SetStream(hip_stream[0]);

    check_error_ret( hipMemcpy2DAsync ((void*)&_Cu[kC*_ldCu], (size_t)(_ldCu * sizeof(T)),
    ...
    check_ret_error( kblas_geqrf_batch( handle,
                                        ...
                                        1));
    if(M < CUV_ncols){
      check_error_ret( hipMemsetAsync( rA,
                                        0,
  ...
  // QR B
  {
    ...
    check_ret_error( kblas_geqrf_batch( handle,
                                        ...
                                        1));
    if(N < CUV_ncols){
      check_error_ret( hipMemsetAsync( rB,
                                        0,
  ...
  //SVD
  {
    ...
    check_ret_error( kblas_acaf_gpu(handle,
      ...
      if(max_acc > 0){
        T dfinal[2];
        check_error_ret( hipMemcpyAsync(&dfinal[0],
                                         pfinalacc,
                                         sizeof(T)*2,
                                         hipMemcpyDeviceToHost,
                                         hip_stream[2]), KBLAS_CUDA_Error );
        hipStreamSynchronize( hip_stream[2] );
    ...
    #else
        // cudaStat = hipMemcpyAsync(d_Crk, pfinalrank, sizeof(int), hipMemcpyDeviceToDevice, hip_stream[2]);
        // cudaStat = hipStreamSynchronize( hip_stream[2] );
        finalrank = CUV_ncols;
    #endif
  ...
  // construct final U
  {
    ...
    if(!clone_CUV){
      check_error_ret( hipMemcpy2DAsync ((void*)Cu, (size_t)(ldCu * sizeof(T)),
  ...
  // construct final U
  {
    ...
    if(!clone_CUV){
      check_error_ret( hipMemcpy2DAsync ((void*)Cv, (size_t)(ldCv * sizeof(T)),
                                          (const void*)newUV, (size_t)(ld_newUV * sizeof(T)),
                                  ...
                                  int max_rk, double max_acc, int batchCount)
{
  ...
  hip_stream[0] = handle->stream;
    hip_stream[1] = handle->streams[0];
    hip_stream[2] = handle->streams[1];
  }else{
    hip_stream[0] = handle->stream;
    hip_stream[1] = handle->stream;
    hip_stream[2] = handle->stream;
  }
  hipEvent_t event_exec[4];
  for (int i = 0; i < 4; ++i)
  {
    check_error_ret( hipEventCreateWithFlags(&event_exec[i], hipEventDisableTiming), KBLAS_CUDA_Error);
  }
  ...
  }
  {
    ...
    hipEventCreateWithFlags(&event_start, hipEventDisableTiming);
    hipEventRecord(event_start, hip_stream[0]);
    hipStreamWaitEvent(hip_stream[1], event_start, 0);
    hipStreamWaitEvent(hip_stream[2], event_start, 0);
  ...
  // QR A
  {
    ...
    check_ret_error( kblas_copy_upper_batch(handle,
                                            ...
                                            rA, ld_rA, stride_rA,
                                            batchCount));
    hipEventRecord(event_exec[0], hip_stream[0]);
  }
  ...
  // QR B
  {
    ...
    hipStreamWaitEvent(hip_stream[1], event_exec[0], 0);
    // rA = rA * rB^T
    check_ret_error( Xtrmm_batch( handle,
                                  ...
                                  batchCount));
    hipEventRecord(event_exec[1], hip_stream[1]);
  }
  ...
  //SVD
  {
    ...
    kblasDestroyRandState(rand_state);
    //*/
    hipStreamWaitEvent(hip_stream[2], event_exec[1], 0);
    ...
    finalrank = kmin(kmin(M,N), finalrank);
    kC = finalrank;
    hipEventRecord(event_exec[2], hip_stream[2]);
  }
  ...
  // ORGQR B
  {
    ...
    check_ret_error( kblas_orgqr_batch( handle,
                                        ...
                                        qrtauB, stride_qrtauB,
                                        batchCount) );
    hipEventRecord(event_exec[3], hip_stream[1]);
  }
  ...
  {
    handle->SetStream(hip_stream[0]);
    hipStreamWaitEvent(hip_stream[0], event_exec[2], 0);
    check_ret_error( Xgemm_batch( handle,
                                  KBLAS_NoTrans, KBLAS_NoTrans,
  ...
  // construct final V
  {
    ...
    hipStreamWaitEvent(hip_stream[0], event_exec[3], 0);
    check_ret_error( Xgemm_batch( handle,
                                  KBLAS_NoTrans, KBLAS_NoTrans,
                            ...
                            hipStream_t hip_stream)
{
  dim3 block(1,1);
  dim3 grid(mt, nt);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_collect_pointers_2d_5<T>), dim3(grid), dim3(block), 0, hip_stream, 
                            ...
                            hipStream_t hip_stream)
{
  dim3 block(1,1);
  dim3 grid(mt, nt);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_collect_pointers_2d_7<T>), dim3(grid), dim3(block), 0, hip_stream, 
                                  output_Au, input_Au,
                            ...
                            hipStream_t hip_stream)
{
  dim3 block(1,1);
  dim3 grid(mt, nt);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_collect_pointers_2d_6<T>), dim3(grid), dim3(block), 0, hip_stream, 
                                  output_Au, (T**)input_Au, output_Av, (T**)input_Av, ld_Aptrs,
                                  output_Bu, (T**)input_Bu, output_Bv, (T**)input_Bv, ld_Bptrs,
