	...
	//main loop
	{
		...
		for(long i = 0; i < total_iterations; i++)
		{
			...
			if(pflag)printf("i = %ld, advance A_\n", i);
			 if(transa == 'n' || transa == 'N') for(long id = 0; id < ngpus_active; id++) {A_[id] = (double*)A + (i *ngpus + id) * tile_size;}
			else for(long id = 0; id < ngpus_active; id++) { A_[id] = (double*)A + (i * ngpus + id) * tile_size * lda; } 
			...
			for(long j = 0; j < n_ ; j++)
			{
				...
				// Advance B_
				if(transb == 'n' || transb == 'N') for(long id = 0; id < ngpus_active; id++) {B_[id] = (double*)B + j * tile_size * ldb;}
				else for(long id = 0; id < ngpus_active; id++) {B_[id] = (double*)B + j * tile_size;}
