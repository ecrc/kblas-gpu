...
int iset_value_diff_1(int* output_array, const int* input_array1, const int* input_array2,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(kernel_set_value_diff_1<int>), dim3(grid), dim3(block), 0, hip_stream, 
                        ...
                        int &max_m, int &max_n, int l)
{
  ...
  hipLaunchKernelGGL(magma_imax_size_kernel_2_copy, dim3(grid), dim3(threads), 0, handle->stream , m, n, max_m, max_n, l);
}
                        ...
                        int &max_m, int &max_n, int &max_k, int l)
{
  dim3 grid(3, 1, 1);
  dim3 threads(AUX_MAX_TX, 1, 1);
  hipLaunchKernelGGL(magma_imax_size_kernel_3_copy, dim3(grid), dim3(threads), 0, handle->stream , m, n, k, max_m, max_n, max_k, l);
}
/*#############################################################################*/
