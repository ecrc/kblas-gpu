...
/**
 * @file testing/batch_triangular/test_Xlauum_batch.cpp
...
template<class T>
int test_Xlauum_batch(kblas_opts& opts)
{
  ...
  for( int itest = 0; itest < opts.ntest; ++itest ) {
    for( int iter = 0; iter < opts.niter; ++iter ) {
      for( int btest = 0; btest < opts.btest; ++btest ) {
        ...
        gflops = batchCount * FLOPS_LAUUM<T>( N ) / 1e9;
        ...
        for(int g = 0; g < ngpu; g++){
          if(strided){
            kblas_lauum_batch_strided_wsquery(kblas_handle[g], N, batchCount_gpu);
          }else{
            kblas_lauum_batch_wsquery(kblas_handle[g], N, batchCount_gpu);
          }
        ...
        if(opts.warmup){
          ...
          for(int g = 0; g < ngpu; g++){
            check_error( cudaSetDevice( opts.devices[g] ));
            if(strided){
              check_kblas_error( kblas_lauum_batch( kblas_handle[g],
                                                    uplo, N,
          ...
          if(opts.time){
            ...
            //   #pragma omp for //schedule(guided,10)
              ...
              {
                int locinfo = 0;
                LAPACK_LAUUM( "L", &N, h_R + s * lda * N, &lda, &locinfo );
                if (locinfo != 0) {
                  printf("%s(%d): dlauum matrix %d returned error %d: %s.\n",
