...
template<class T>
int test_Xposv_batch(kblas_opts& opts)
{
  ...
  for( int itest = 0; itest < opts.ntest; ++itest ) {
    for( int iter = 0; iter < opts.niter; ++iter ) {
      for( int btest = 0; btest < opts.btest; ++btest ) {
        ...
        gflops = batchCount * FLOPS_POSV<T>(opts.side, M, N ) / 1e9;
        ...
        for(int r = 0;  r < nruns; r++){
          ...
          for(int g = 0; g < ngpu; g++){
            check_error( cudaSetDevice( opts.devices[g] ));
            if(strided){
              check_kblas_error( kblas_posv_batch(kblas_handle[g],
                                                  opts.side, opts.uplo,
                                                  Cm, Cn,
                                                  (T*)(d_A[g]), ldda, An*ldda,
                                                  (T*)(d_C[g]), lddc, Cn*lddc,
                                                  batchCount_gpu,
                                                  d_info[g]) );
            }else{
              check_kblas_error( kblas_posv_batch(kblas_handle[g],
          ...
          for(int r = 0; r < nruns; r++)
          {
            ...
            for (int s=0; s < batchCount; s++)
            {
              ...
              LAPACK_TRSM( (( opts.side == KBLAS_Right ) ? "Right" : "Left"),
                          (( opts.uplo == KBLAS_Lower ) ? "Lower" : "Upper"),
                          "Transpose", "Non-unit",
                           &Cm, &Cn, &c_one, h_A + s * lda * An, &lda, h_C + s * ldc * Cn, &ldc );
              LAPACK_TRSM( (( opts.side == KBLAS_Right ) ? "Right" : "Left"),
                          (( opts.uplo == KBLAS_Lower ) ? "Lower" : "Upper"),
