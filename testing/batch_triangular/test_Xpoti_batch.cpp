...
/**
 * @file testing/batch_triangular/test_Xpoti_batch.cpp
...
template<class T>
int test_Xpoti_batch(kblas_opts& opts)
{
  ...
  for( int itest = 0; itest < opts.ntest; ++itest ) {
    for( int iter = 0; iter < opts.niter; ++iter ) {
      for( int btest = 0; btest < opts.btest; ++btest ) {
        ...
        gflops = batchCount * FLOPS_POTI<T>( N ) / 1e9;
        ...
        for(int g = 0; g < ngpu; g++){
          if(strided){
            kblas_poti_batch_strided_wsquery(kblas_handle[g], Am, batchCount_gpu);
          }else{
            kblas_poti_batch_wsquery(kblas_handle[g], Am, batchCount_gpu);
          }
        ...
        for(int r = 0;  r < nruns; r++){
          ...
          for(int g = 0; g < ngpu; g++){
            check_error( cudaSetDevice( opts.devices[g] ));
            if(strided){
              check_kblas_error( kblas_poti_batch( kblas_handle[g],
                                                    uplo, N,
                                                    d_A[g], ldda, An*ldda,
                                                    batchCount_gpu,
                                                    d_info[g]) );
          ...
          for(int r = 0; r < nruns; r++)
          {
            ...
            for (int s=0; s < batchCount; s++)
            {
              ...
              LAPACK_POTRI( (( opts.uplo == KBLAS_Lower ) ? "Lower" : "Upper"),
                            &N, h_R + s * lda * N, &lda, &locinfo );
              if (locinfo != 0) {
                  printf("%s(%d): dpoti matrix %d returned error %d: %s.\n",
