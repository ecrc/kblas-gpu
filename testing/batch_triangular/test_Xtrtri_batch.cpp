...
template<class T>
int test_Xtrtri_batch(kblas_opts& opts)
{
  ...
  for( int itest = 0; itest < opts.ntest; ++itest ) {
    for( int iter = 0; iter < opts.niter; ++iter ) {
      for( int btest = 0; btest < opts.btest; ++btest ) {
        ...
        gflops = batchCount * FLOPS_TRTRI<T>( N ) / 1e9;
        ...
        for(int g = 0; g < ngpu; g++){
          if(strided){
            ...
            kblas_trtri_batch_strided_wsquery(kblas_handle[g], Am, batchCount_gpu);
          }else{
            kblas_potrf_batch_wsquery(kblas_handle[g], Am, batchCount_gpu);
            kblas_trtri_batch_wsquery(kblas_handle[g], Am, batchCount_gpu);
          }
        ...
        if(opts.check || (opts.time && opts.lapack)){
          ...
          for(int r = 0; r < nruns; r++)
          {
            ...
            for (int s=0; s < batchCount; s++)
            {
              ...
              int locinfo = 0;
              // printf("N(%d), lda(%d)\n", N, lda);
              LAPACK_TRTRI( (( opts.uplo == KBLAS_Lower ) ? "Lower" : "Upper"),
                            "N", &N, h_R + s * lda * N, &lda, &locinfo );
              if (locinfo != 0) {
                  printf("%s(%d): dtrtri matrix %d returned error %d: %s.\n",
