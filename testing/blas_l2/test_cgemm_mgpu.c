...
int main(int argc, char** argv)
{
    ...
    cuFloatComplex* B = NULL;
    cuFloatComplex* C = NULL;
    cuFloatComplex* csingle = NULL;
    cuFloatComplex* cmgpu = NULL;
	...
	// cpu alloc / init
	{
    	...
    	//A = (cuFloatComplex*)malloc(size_a*sizeof(cuFloatComplex));
    	//B = (cuFloatComplex*)malloc(size_b*sizeof(cuFloatComplex));
    	C = (cuFloatComplex*)malloc(size_c*sizeof(cuFloatComplex));
    	csingle = (cuFloatComplex*)malloc(size_c*sizeof(cuFloatComplex));
    	//cmgpu = (cuFloatComplex*)malloc(size_c*sizeof(cuFloatComplex));

    	cudaMallocHost((void**)&A, size_a*sizeof(cuFloatComplex));			// better for mgpu version
    	cudaMallocHost((void**)&B, size_b*sizeof(cuFloatComplex));			// better for mgpu version
    	//cudaMallocHost((void**)&C, size_c*sizeof(cuFloatComplex));			// better for mgpu version
    	//cudaMallocHost((void**)&csingle, size_c*sizeof(cuFloatComplex));	// better for mgpu version
    	cudaMallocHost((void**)&cmgpu, size_c*sizeof(cuFloatComplex));		// better for mgpu version
	...
	for(ngpus = ngpus1; ngpus <= ngpus2; ngpus++)
	{
		...
		for(dim = istart; dim <= istop; dim += istep)
		{
			...
			// single gpu test
			{
    			...
    			cuFloatComplex* dB_single = NULL;
    			cuFloatComplex* dC_single = NULL;
    			...
    			cudaSetDevice(gpu_id[0]);
    			e1 = cudaMalloc((void**)&dA_single, sizea*sizeof(cuFloatComplex));
    			e2 = cudaMalloc((void**)&dB_single, sizeb*sizeof(cuFloatComplex));
    			e3 = cudaMalloc((void**)&dC_single, sizec*sizeof(cuFloatComplex));
    			...
    			if(alloc == 1)
    			{
    				...
    				cublasSetMatrix((long)m, (long)k, sizeof(cuFloatComplex), A, (long)LDA, dA_single, (long)lda);
    				cublasSetMatrix((long)k, (long)n, sizeof(cuFloatComplex), B, (long)LDB, dB_single, (long)ldb);
    				float time = 0.0;
    				long r;
    				for(r = 0; r < (NRUNS+1); r++)
    				{
    					cublasSetMatrix((long)m, (long)n, sizeof(cuFloatComplex), C, (long)LDC, dC_single, (long)ldc);
					...
					cublasGetMatrix((long)m, (long)n, sizeof(cuFloatComplex), dC_single, (long)ldc, csingle, (long)LDC);
			...
			// mgpu test
			{
				...
				{
    				// make a copy of C
    				memcpy(cmgpu, C, size_c * sizeof(cuFloatComplex));
    		...
    		// testing correctness
    		{
    			if(alloc == 1 && alloc_mgpu == 1)
    			{
    				// testing error -- specify ref. vector and result vector
      				cuFloatComplex* cref = csingle;
      				cuFloatComplex* cres = cmgpu;
