...
/**
 * @file testing/blas_l2/test_cgemm_mgpu.c
...
int main(int argc, char** argv)
{
	...
	hipFloatComplex alpha = make_hipFloatComplex(1.0, 0.0);
	hipFloatComplex beta = make_hipFloatComplex(0.0, 0.0);
    ...
    hipFloatComplex* B = NULL;
    hipFloatComplex* C = NULL;
    hipFloatComplex* csingle = NULL;
    hipFloatComplex* cmgpu = NULL;
	...
	// cpu alloc / init
	{
    	...
    	//A = (hipFloatComplex*)malloc(size_a*sizeof(hipFloatComplex));
    	//B = (hipFloatComplex*)malloc(size_b*sizeof(hipFloatComplex));
    	C = (hipFloatComplex*)malloc(size_c*sizeof(hipFloatComplex));
    	csingle = (hipFloatComplex*)malloc(size_c*sizeof(hipFloatComplex));
    	//cmgpu = (hipFloatComplex*)malloc(size_c*sizeof(hipFloatComplex));

    	hipHostMalloc((void**)&A, size_a*sizeof(hipFloatComplex));			// better for mgpu version
    	hipHostMalloc((void**)&B, size_b*sizeof(hipFloatComplex));			// better for mgpu version
    	//hipHostMalloc((void**)&C, size_c*sizeof(hipFloatComplex));			// better for mgpu version
    	//hipHostMalloc((void**)&csingle, size_c*sizeof(hipFloatComplex));	// better for mgpu version
    	hipHostMalloc((void**)&cmgpu, size_c*sizeof(hipFloatComplex));		// better for mgpu version


    	printf("Initializing on cpu .. \n");
    	// Initialize matrix and vector on cpu
    	long i, j, m;
    	crand_matrix(M, K, A, LDA);
    	crand_matrix(K, N, B, LDB);
    	crand_matrix(M, N, C, LDC);
	}
	...
	for(ngpus = ngpus1; ngpus <= ngpus2; ngpus++)
	{
		...
		for(dim = istart; dim <= istop; dim += istep)
		{
			...
			float flops = FLOPS_CGEMM(dimf, dimf, dimf) / 1e6;
			...
			// single gpu test
			{
    			...
    			hipFloatComplex* dB_single = NULL;
    			hipFloatComplex* dC_single = NULL;
    			...
    			hipError_t e1, e2, e3;
    			hipSetDevice(gpu_id[0]);
    			e1 = hipMalloc((void**)&dA_single, sizea*sizeof(hipFloatComplex));
    			e2 = hipMalloc((void**)&dB_single, sizeb*sizeof(hipFloatComplex));
    			e3 = hipMalloc((void**)&dC_single, sizec*sizeof(hipFloatComplex));
    			...
    			if(alloc == 1)
    			{
    				...
    				hipblasSetMatrix((long)m, (long)k, sizeof(hipFloatComplex), A, (long)LDA, dA_single, (long)lda);
    				hipblasSetMatrix((long)k, (long)n, sizeof(hipFloatComplex), B, (long)LDB, dB_single, (long)ldb);
    				float time = 0.0;
    				long r;
    				for(r = 0; r < (NRUNS+1); r++)
    				{
    					hipblasSetMatrix((long)m, (long)n, sizeof(hipFloatComplex), C, (long)LDC, dC_single, (long)ldc);
    					//printf("check point 1.3\n");
    					hipSetDevice(gpu_id[0]);
      					hipEventRecord(start, 0);
      					// call cublas cgemm
      					hipblasCgemm(transa, transb,
					...
					hipblasGetMatrix((long)m, (long)n, sizeof(hipFloatComplex), dC_single, (long)ldc, csingle, (long)LDC);
			...
			// mgpu test
			{
				...
				{
    				// make a copy of C
    				memcpy(cmgpu, C, size_c * sizeof(hipFloatComplex));

    				hipSetDevice(gpu_id[0]);
      				hipEventRecord(start, 0);
      				// cgemm_mgpu
      				tile = tile_size;
      				kblas_cgemm_mgpu( transa, transb,
      							(long)m, (long)n, (long)k,
      							alpha, A, (long)LDA,
      							B, (long)LDB,
      							beta, cmgpu, (long)LDC,
    		...
    		// testing correctness
    		{
    			if(alloc == 1 && alloc_mgpu == 1)
    			{
    				// testing error -- specify ref. vector and result vector
      				hipFloatComplex* cref = csingle;
      				hipFloatComplex* cres = cmgpu;
      				//error = get_max_error(cref, cres, m , n, LDC);
    				error = cget_max_error_matrix(cref, cres, m, n, LDC);
    				printf("%-10e;\n", error);
    			}
    			else
    				printf ("N/A       \n");
