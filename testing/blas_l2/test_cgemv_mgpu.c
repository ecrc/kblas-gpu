...
/**
 * @file testing/blas_l2/test_cgemv_mgpu.c
...
int main(int argc, char** argv)
{
	...
	hipFloatComplex alpha = make_hipFloatComplex(2.3, 0);
	hipFloatComplex beta  = make_hipFloatComplex(-1.0, 0);
    ...
    hipFloatComplex* y = NULL;
    hipFloatComplex* ysingle = NULL;
    hipFloatComplex* ykblas[MAX_NGPUS] = {NULL};
	...
	// cpu alloc / init
	{
    	...
    	y = (hipFloatComplex*)malloc(vecsize_y*sizeof(hipFloatComplex));
    	ysingle = (hipFloatComplex*)malloc(vecsize_y*sizeof(hipFloatComplex));
    	for(k = 0; k < ngpus; k++)
    		ykblas[k] = (hipFloatComplex*)malloc(vecsize_y*sizeof(hipFloatComplex));
    	...
    	for(i = 0; i < vecsize_x; i++) x[i] = kblas_crand();

    	for(i = 0; i < vecsize_y; i++) y[i] = kblas_crand();
	}
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
			hipFloatComplex* dA_single = NULL;
    		hipFloatComplex* dx_single = NULL;
    		hipFloatComplex* dy_single = NULL;
    		...
    		hipError_t e1, e2, e3;
    		hipSetDevice(0);
    		e1 = hipMalloc((void**)&dA_single, n*lda_*sizeof(hipFloatComplex));
    		e2 = hipMalloc((void**)&dx_single, vecsize_x*sizeof(hipFloatComplex));
    		e3 = hipMalloc((void**)&dy_single, vecsize_y*sizeof(hipFloatComplex));
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			hipSetDevice(0);
    			hipblasSetMatrix(m, n, sizeof(hipFloatComplex), A, LDA, dA_single, lda_);

    			float time = 0;
    			elapsedTime = 0.0;
    			for(r = 0; r < nruns; r++)
    			{
    				hipMemcpy(dy_single, y, vecsize_y*sizeof(hipFloatComplex), hipMemcpyHostToDevice);
    				hipMemcpy(dx_single, x, vecsize_x*sizeof(hipFloatComplex), hipMemcpyHostToDevice);

    				// handle offset
    				hipFloatComplex* da = dA_single + (offset * lda_) + offset;
    				hipFloatComplex* dx = dx_single + (offset * incx);
    				hipFloatComplex* dy = dy_single + (offset * incy);
    				int m_ = m-offset;
    				int n_ = n-offset;

    				hipSetDevice(0);
      				hipEventRecord(start, 0);
      				//hipblasStatus_t s = hipblasCgemv(cublas_handle, trans_, m, n, &alpha, dA_single, lda_, dx_single, incx, &beta, dy_single, incy);
      				kblas_cgemv(trans, m_, n_, alpha, da, lda_, dx, incx, beta, dy, incy);
				...
				hipMemcpy(ysingle, dy_single, vecsize_y * sizeof(hipFloatComplex), hipMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		hipFloatComplex* dA[MAX_NGPUS] = {NULL};
    		hipFloatComplex* dx[MAX_NGPUS] = {NULL};
    		hipFloatComplex* dy[MAX_NGPUS] = {NULL};
    		...
    		// for kblas test
    		// alloc
    		int nb = get_cgemv_mgpu_bs(trans);
    		...
    		{
    			hipSetDevice(k);
    			e1 = hipMalloc((void**)&dx[k], vecsize_x*sizeof(hipFloatComplex));
    			e2 = hipMalloc((void**)&dy[k], vecsize_y*sizeof(hipFloatComplex));
    			if( (e1 != hipSuccess) || (e2 != hipSuccess) )
    			{
    				alloc_mgpu = 0; break;
    			}
    			// init y
    			hipMemset(dy[k], 0, vecsize_y*sizeof(hipFloatComplex));
    		}
    		...
    		if(alloc_mgpu == 1)
    		{
    			...
    			{
    				hipSetDevice(k);
    				hipError_t e = hipMemcpy(dx[k], x, vecsize_x*sizeof(hipFloatComplex), hipMemcpyHostToDevice);
				}

				// offload A and y on gpus
				kblas_csetmatrix_mgpu_1D(m, n, A, LDA, dA, lda_, ngpus, nb);
				float time = 0;
				float _time = 0;
				elapsedTime = 0.0;
				for(r = 0; r < nruns; r++)
				{
				    // broadcast y to all gpus
				    for(k = 0; k < ngpus; k++)
    			    {
    				    hipSetDevice(k);
    				    hipError_t e = hipMemcpy(dy[k], y, vecsize_y*sizeof(hipFloatComplex), hipMemcpyHostToDevice);
				    }
    				...
    				// ---- kblas
      				...
      				kblas_cgemv_mgpu_async(trans, m, n, alpha, dA, lda_, dx, incx, beta, dy, incy, ngpus, offset, offset, streams);
      			...
      			{
      				hipSetDevice(k);
      				hipMemcpy(ykblas[k], dy[k], vecsize_y * sizeof(hipFloatComplex), hipMemcpyDeviceToHost);
      			}
      			elapsedTime /= nruns;
    	...
    	// testing correctness
    	{
    		if(alloc == 1 && alloc_mgpu == 1)
    		{
    			// testing error -- specify ref. vector and result vector
      			hipFloatComplex* yref = ysingle + (offset * incy);
      			hipFloatComplex* yres = ykblas[0] + (offset * incy);
      			error = cget_max_error(yref, yres, m-offset, incy);
    			//print
    			printf("%-10e;\n", error);
    		}
    		else
