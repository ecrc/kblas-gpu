...
int main(int argc, char** argv)
{
	...
	cuFloatComplex alpha = make_cuFloatComplex(2.3, 0);
	cuFloatComplex beta  = make_cuFloatComplex(-1.0, 0);
	...
	// cpu alloc / init
	{
    	...
    	y = (cuFloatComplex*)malloc(vecsize_y*sizeof(cuFloatComplex));
    	ysingle = (cuFloatComplex*)malloc(vecsize_y*sizeof(cuFloatComplex));
    	for(k = 0; k < ngpus; k++)
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			cudaSetDevice(0);
    			cublasSetMatrix(m, n, sizeof(cuFloatComplex), A, LDA, dA_single, lda_);
    			...
    			for(r = 0; r < nruns; r++)
    			{
    				...
    				cudaSetDevice(0);
      				...
      				//cublasStatus_t s = cublasCgemv(cublas_handle, trans_, m, n, &alpha, dA_single, lda_, dx_single, incx, &beta, dy_single, incy);
				...
				cudaMemcpy(ysingle, dy_single, vecsize_y * sizeof(cuFloatComplex), cudaMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		if(alloc_mgpu == 1)
    		{
    			...
    			{
    				cudaSetDevice(k);
    				cudaError_t e = cudaMemcpy(dx[k], x, vecsize_x*sizeof(cuFloatComplex), cudaMemcpyHostToDevice);
				}
				...
				for(r = 0; r < nruns; r++)
				{
    			    ...
    			    {
    				    cudaSetDevice(k);
    				    cudaError_t e = cudaMemcpy(dy[k], y, vecsize_y*sizeof(cuFloatComplex), cudaMemcpyHostToDevice);
				    }
    	...
    	// testing correctness
    	{
    		if(alloc == 1 && alloc_mgpu == 1)
    		{
    			// testing error -- specify ref. vector and result vector
      			cuFloatComplex* yref = ysingle + (offset * incy);
      			cuFloatComplex* yres = ykblas[0] + (offset * incy);
