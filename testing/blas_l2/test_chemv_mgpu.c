...
int main(int argc, char** argv)
{
	...
	// cpu alloc / init
	{
    	...
    	y = (hipFloatComplex*)malloc(vecsize_x*sizeof(hipFloatComplex));
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			hipSetDevice(0);
    			hipblasSetMatrix(m, m, sizeof(hipFloatComplex), A, LDA, dA_single, lda_);
    			...
    			for(r = 0; r < nruns; r++)
    			{
    				...
    				hipSetDevice(0);
      				...
      				//hipblasStatus_t s = cublasSsymv(cublas_handle, uplo_, m, &alpha, dA_single, lda_, dx_single, incx, &beta, dy_single, incy);
				...
				hipMemcpy(ycuda, dy_single, vecsize_y * sizeof(hipFloatComplex), hipMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		if(alloc_mgpu == 1)
    		{
    			...
    			{
    				hipSetDevice(k);
    				hipMemcpy(dx[k], x, vecsize_x*sizeof(hipFloatComplex), hipMemcpyHostToDevice);
				}
