    ...
    for(dim = istart; dim <= istop; dim += istep)
    {
      ...
      // single gpu test
      {
        ...
        if(alloc == 1)
        {
          ...
          cublasSetMatrix((long)m, (long)k, sizeof(double), A, (long)LDA, dA_single, (long)lda);
          cublasSetMatrix((long)k, (long)n, sizeof(double), B, (long)LDB, dB_single, (long)ldb);
          ...
          for(r = 0; r < (NRUNS+1); r++)
          {
            cudaSetDevice(gpu_id[0]);
            cudaEventRecord(start, 0);
            cublasSetMatrix((long)m, (long)n, sizeof(double), C, (long)LDC, dC_single, (long)ldc);
          ...
          cudaEventRecord(start, 0);
          cublasGetMatrix((long)m, (long)n, sizeof(double), dC_single, (long)ldc, csingle, (long)LDC);
          cudaEventRecord(stop, 0);
          cudaEventSynchronize(stop);
          elapsedTime = 0.0;
          cudaEventElapsedTime(&elapsedTime, start, stop);
