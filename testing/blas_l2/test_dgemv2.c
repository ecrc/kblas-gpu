...
/**
 * @file testing/blas_l2/test_dgemv2.c
...
int main(int argc, char** argv)
{
    ...
    for(m = istart; m <= istop; m += istep)
    {
		...
		hipblasSetMatrix(m, n, sizeof(double), A, LDA, dA, lda);
		...
		for(i = 0; i < vecsize_y; i++)
    	{
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		...
      		hipblasDgemv(cublas_handle, trans_, m, n, &alpha, dA, lda, dx, incx, &beta, dy, incy);
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		hipMemcpy(dy, ykblas2, vecsize_y * sizeof(double), hipMemcpyHostToDevice);
      	...
      	hipMemcpy(ykblas2, dy, vecsize_y * sizeof(double), hipMemcpyDeviceToHost);
