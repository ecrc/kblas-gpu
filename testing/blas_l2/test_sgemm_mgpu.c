...
int main(int argc, char** argv)
{
	...
	// cpu alloc / init
	{
    	...
    	hipHostMalloc((void**)&A, size_a*sizeof(float));			// better for mgpu version
    	hipHostMalloc((void**)&B, size_b*sizeof(float));			// better for mgpu version
    	//hipHostMalloc((void**)&C, size_c*sizeof(float));			// better for mgpu version
    	//hipHostMalloc((void**)&csingle, size_c*sizeof(float));	// better for mgpu version
    	hipHostMalloc((void**)&cmgpu, size_c*sizeof(float));		// better for mgpu version
	...
	for(ngpus = ngpus1; ngpus <= ngpus2; ngpus++)
	{
		...
		for(dim = istart; dim <= istop; dim += istep)
		{
			...
			// single gpu test
			{
    			...
    			hipError_t e1, e2, e3;
    			hipSetDevice(gpu_id[0]);
    			e1 = hipMalloc((void**)&dA_single, sizea*sizeof(float));
    			e2 = hipMalloc((void**)&dB_single, sizeb*sizeof(float));
    			e3 = hipMalloc((void**)&dC_single, sizec*sizeof(float));
    			...
    			if(alloc == 1)
    			{
    				...
    				hipblasSetMatrix((long)m, (long)k, sizeof(float), A, (long)LDA, dA_single, (long)lda);
    				hipblasSetMatrix((long)k, (long)n, sizeof(float), B, (long)LDB, dB_single, (long)ldb);
    				float time = 0.0;
    				long r;
    				for(r = 0; r < (NRUNS+1); r++)
    				{
    					hipblasSetMatrix((long)m, (long)n, sizeof(float), C, (long)LDC, dC_single, (long)ldc);
					...
					hipblasGetMatrix((long)m, (long)n, sizeof(float), dC_single, (long)ldc, csingle, (long)LDC);
