...
/**
 * @file testing/blas_l2/test_sgemv2_dim.c
...
int main(int argc, char** argv)
{
    ...
    // remove the for loop
    {
		...
		hipblasSetMatrix(m, n, sizeof(float), A, LDA, dA, lda);
		...
		for(i = 0; i < vecsize_y; i++)
    	{
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		...
      		hipblasSgemv(cublas_handle, trans_, m, n, &alpha, dA, lda, dx, incx, &beta, dy, incy);
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		hipMemcpy(dy, ykblas2, vecsize_y * sizeof(float), hipMemcpyHostToDevice);
      	...
      	hipMemcpy(ykblas2, dy, vecsize_y * sizeof(float), hipMemcpyDeviceToHost);
