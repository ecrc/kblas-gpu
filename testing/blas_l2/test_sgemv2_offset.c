...
/**
 * @file testing/blas_l2/test_sgemv2_offset.c
...
int main(int argc, char** argv)
{
    ...
    hipblasSetMatrix(dim, dim, sizeof(float), A, LDA, dA, LDA_);
    ...
    for(m = istart; m <= istop; m += istep)
    {
        ...
        int vecsize_x_ = vecsize_x - offset;
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		hipMemcpy(dy_, ycuda, vecsize_y_ * sizeof(float), hipMemcpyHostToDevice);

      		hipEventRecord(start, 0);
      		hipblasSgemv(cublas_handle, trans_, dim_, dim_, &alpha, dA_, LDA_, dx_, incx, &beta, dy_, incy);
      	...
      	hipMemcpy(ycuda, dy_, vecsize_y_ * sizeof(float), hipMemcpyDeviceToHost);
      	// end of cuda test
      	...
      	for(r = 0; r < nruns; r++)
      	{
      		hipMemcpy(dy_, ykblas, vecsize_y_ * sizeof(float), hipMemcpyHostToDevice);

      		hipEventRecord(start, 0);
      		kblas_sgemv2_offset( trans, dim, dim, alpha, dA, LDA_, dx, incx, beta, dy, incy, offset, offset);
      		hipEventRecord(stop, 0);
      		hipEventSynchronize(stop);
      	...
      	hipMemcpy(ykblas, dy_, vecsize_y_ * sizeof(float), hipMemcpyDeviceToHost);
