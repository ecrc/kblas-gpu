...
int main(int argc, char** argv)
{
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		hipError_t e1, e2, e3;
    		hipSetDevice(0);
    		e1 = hipMalloc((void**)&dA_single, n*lda_*sizeof(float));
    		e2 = hipMalloc((void**)&dx_single, vecsize_x*sizeof(float));
    		e3 = hipMalloc((void**)&dy_single, vecsize_y*sizeof(float));
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			hipSetDevice(0);
    			hipblasSetMatrix(m, n, sizeof(float), A, LDA, dA_single, lda_);

    			float time = 0;
    			elapsedTime = 0.0;
    			for(r = 0; r < nruns; r++)
    			{
    				hipMemcpy(dy_single, y, vecsize_y*sizeof(float), hipMemcpyHostToDevice);
    				hipMemcpy(dx_single, x, vecsize_x*sizeof(float), hipMemcpyHostToDevice);
    				...
    				hipSetDevice(0);
      				...
      				//hipblasStatus_t s = hipblasSgemv(cublas_handle, trans_, m, n, &alpha, dA_single, lda_, dx_single, incx, &beta, dy_single, incy);
				...
				hipMemcpy(ysingle, dy_single, vecsize_y * sizeof(float), hipMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		{
    			hipSetDevice(k);
    			e1 = hipMalloc((void**)&dx[k], vecsize_x*sizeof(float));
    			e2 = hipMalloc((void**)&dy[k], vecsize_y*sizeof(float));
    			if( (e1 != hipSuccess) || (e2 != hipSuccess) )
    			{
    				alloc_mgpu = 0; break;
    			}
    			// init y
    			hipMemset(dy[k], 0, vecsize_y*sizeof(float));
    		}
    		...
    		if(alloc_mgpu == 1)
    		{
    			...
    			{
    				hipSetDevice(k);
    				hipError_t e = hipMemcpy(dx[k], x, vecsize_x*sizeof(float), hipMemcpyHostToDevice);
				...
				for(r = 0; r < nruns; r++)
				{
    			    ...
    			    {
    				    hipSetDevice(k);
    				    hipError_t e = hipMemcpy(dy[k], y, vecsize_y*sizeof(float), hipMemcpyHostToDevice);
					}
      			...
      			{
      				hipSetDevice(k);
      				hipMemcpy(ykblas[k], dy[k], vecsize_y * sizeof(float), hipMemcpyDeviceToHost);
      			}
      			elapsedTime /= nruns;
