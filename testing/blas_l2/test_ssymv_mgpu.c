...
/**
 * @file testing/blas_l2/test_ssymv_mgpu.c
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			hipSetDevice(0);
    			hipblasSetMatrix(m, m, sizeof(float), A, LDA, dA_single, lda_);
    			...
    			for(r = 0; r < nruns; r++)
    			{
    				...
    				hipSetDevice(0);
      				...
      				//hipblasStatus_t s = cublasSsymv(cublas_handle, uplo_, m_, &alpha, da, lda_, dx, incx, &beta, dy, incy);
				...
				hipMemcpy(ycuda, dy_single, vecsize_y * sizeof(float), hipMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		if(alloc_mgpu == 1)
    		{
    			...
    			{
    				hipSetDevice(k);
    				hipMemcpy(dx[k], x, vecsize_x*sizeof(float), hipMemcpyHostToDevice);
				}
