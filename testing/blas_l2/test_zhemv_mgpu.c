...
/**
 * @file testing/blas_l2/test_zhemv_mgpu.c
...
int main(int argc, char** argv)
{
	...
	cuDoubleComplex alpha = make_cuDoubleComplex(1.0, -0.6);
	...
	// cpu alloc / init
	{
    	...
    	y = (cuDoubleComplex*)malloc(vecsize_x*sizeof(cuDoubleComplex));
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			cudaSetDevice(0);
    			cublasSetMatrix(m, m, sizeof(cuDoubleComplex), A, LDA, dA_single, lda_);
    			...
    			for(r = 0; r < nruns; r++)
    			{
    				...
    				cudaSetDevice(0);
      				...
      				kblas_zhemv(uplo, m_, alpha, da, lda_, dx, incx, beta, dy, incy);
				...
				cudaMemcpy(ycuda, dy_single, vecsize_y * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		// alloc
    		alloc_mgpu = 1;
    		int nb = get_zhemv_mgpu_bs(uplo);
    		kblas_zmalloc_mgpu_1D(m, m, dA, ngpus, lda_, nb);
    		...
    		{
    			cudaSetDevice(k);
    			e1 = cudaMalloc((void**)&dx[k], vecsize_x*sizeof(cuDoubleComplex));
    			e2 = cudaMalloc((void**)&dy[k], vecsize_y*sizeof(cuDoubleComplex));
    		...
    		if(alloc_mgpu  == 1)
    		{
    			...
    			{
    				cudaSetDevice(k);
    				cudaMemcpy(dx[k], x, vecsize_x*sizeof(cuDoubleComplex), cudaMemcpyHostToDevice);
				}

				// offload A and y on gpus
				kblas_zsetmatrix_mgpu_1D(m, m, A, LDA, dA, lda_, ngpus, nb);
    			float time = 0;
				float _time = 0;
				elapsedTime = 0;
				for(r = 0; r < nruns; r++)
				{
					...
					kblas_zsetvector_mgpu_1D(m, y, dy, ngpus, nb);
    				...
    				// ---- kblas
      				...
      				kblas_zhemv_mgpu_async(uplo, m, alpha, dA, lda_, dx, incx, beta, dy, incy, ngpus, offset, streams);
      			...
      			{
      				cudaSetDevice(k);
      				cudaMemcpy(ykblas[k], dy[k], vecsize_y * sizeof(cuDoubleComplex), cudaMemcpyDeviceToHost);
      			}
    	...
    	// testing correctness
    	{
    		if(alloc == 1 && alloc_mgpu == 1)
    		{
    			// testing error -- specify ref. vector and result vector
      			...
      			cuDoubleComplex* yres = ykblas[0];

      			error = zget_max_error(yref, yres, m, incy);
    			//print
    			printf("%-10e;\n", error);
    		}
    		else
