...
int main(int argc, char** argv)
{
	...
	hipDoubleComplex alpha = make_hipDoubleComplex(1.0, -0.6);
	...
	// cpu alloc / init
	{
    	...
    	y = (hipDoubleComplex*)malloc(vecsize_x*sizeof(hipDoubleComplex));
	...
	for(dim = istart; dim <= istop; dim += istep)
	{
		...
		// single gpu test
		{
    		...
    		if(alloc == 1)
    		{
    			// offload A, y to gpus
    			hipSetDevice(0);
    			hipblasSetMatrix(m, m, sizeof(hipDoubleComplex), A, LDA, dA_single, lda_);
				...
				hipMemcpy(ycuda, dy_single, vecsize_y * sizeof(hipDoubleComplex), hipMemcpyDeviceToHost);
		...
		// mgpu test
		{
    		...
    		if(alloc_mgpu  == 1)
    		{
    			...
    			{
    				hipSetDevice(k);
    				hipMemcpy(dx[k], x, vecsize_x*sizeof(hipDoubleComplex), hipMemcpyHostToDevice);
				}
    	...
    	// testing correctness
    	{
    		if(alloc == 1 && alloc_mgpu == 1)
    		{
    			// testing error -- specify ref. vector and result vector
      			...
      			hipDoubleComplex* yres = ykblas[0];
