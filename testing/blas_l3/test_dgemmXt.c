...
int main(int argc, char* argv[]){
  ...
  if(cublasXtCreate(&cublasXt_handle) != HIPBLAS_STATUS_SUCCESS) {printf("handle create fail\n"); return 1;}
  //int devices[8] = { 0,1,2,3,4, 5, 6, 7 };  // add this line
  if((status = cublasXtDeviceSelect(cublasXt_handle, opts.ngpu, opts.devices)) != HIPBLAS_STATUS_SUCCESS) {
  ...
  for( int i = 0; i < opts.ntest; ++i ) {
    ...
    if ( hipHostMalloc((void**)&h_A, (lda*An)*sizeof( double ),0 ) != hipSuccess) {
      fprintf( stderr, "!!!! hipHostMalloc failed for: h_A\n" );
      exit(-1);
    }
    if ( hipHostMalloc((void**)&h_B, (ldb*Bn)*sizeof( double ),0 ) != hipSuccess) {
      fprintf( stderr, "!!!! hipHostMalloc failed for: h_B\n" );
      exit(-1);
    }
    if ( hipHostMalloc((void**)&h_C, (ldc*N)*sizeof( double ),0 ) != hipSuccess) {
      fprintf( stderr, "!!!! hipHostMalloc failed for: h_C\n" );
      exit(-1);
    }
    /*if ( hipHostMalloc((void**)&h_R, (ldc*N)*sizeof( double ),0 ) != hipSuccess) {
      fprintf( stderr, "!!!! hipHostMalloc failed for: h_C\n" );
      exit(-1);
    }*/
