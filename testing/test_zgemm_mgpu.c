...
int main(int argc, char** argv)
{
    ...
    cuDoubleComplex* csingle = NULL;
    cuDoubleComplex* cmgpu = NULL;
	...
	// cpu alloc / init
	{
    	...
    	//A = (cuDoubleComplex*)malloc(size_a*sizeof(cuDoubleComplex));
    	//B = (cuDoubleComplex*)malloc(size_b*sizeof(cuDoubleComplex));
    	C = (cuDoubleComplex*)malloc(size_c*sizeof(cuDoubleComplex));
    	csingle = (cuDoubleComplex*)malloc(size_c*sizeof(cuDoubleComplex));
    	//cmgpu = (cuDoubleComplex*)malloc(size_c*sizeof(cuDoubleComplex));
    	
    	cudaMallocHost((void**)&A, size_a*sizeof(cuDoubleComplex));			// better for mgpu version
    	cudaMallocHost((void**)&B, size_b*sizeof(cuDoubleComplex));			// better for mgpu version
    	//cudaMallocHost((void**)&C, size_c*sizeof(cuDoubleComplex));			// better for mgpu version
    	//cudaMallocHost((void**)&csingle, size_c*sizeof(cuDoubleComplex));	// better for mgpu version
    	cudaMallocHost((void**)&cmgpu, size_c*sizeof(cuDoubleComplex));		// better for mgpu version
	...
	for(ngpus = ngpus1; ngpus <= ngpus2; ngpus++)
	{
		...
		for(dim = istart; dim <= istop; dim += istep)
		{
			...
			// single gpu test
			{
    			...
    			cuDoubleComplex* dB_single = NULL;
    			cuDoubleComplex* dC_single = NULL;
    			...
    			cudaSetDevice(gpu_id[0]);
    			e1 = cudaMalloc((void**)&dA_single, sizea*sizeof(cuDoubleComplex));
    			e2 = cudaMalloc((void**)&dB_single, sizeb*sizeof(cuDoubleComplex));
    			e3 = cudaMalloc((void**)&dC_single, sizec*sizeof(cuDoubleComplex));
    			...
    			if(alloc == 1)
    			{
    				...
    				cublasSetMatrix((long)m, (long)k, sizeof(cuDoubleComplex), A, (long)LDA, dA_single, (long)lda);
    				cublasSetMatrix((long)k, (long)n, sizeof(cuDoubleComplex), B, (long)LDB, dB_single, (long)ldb);
    				float time = 0.0;
    				long r;
    				for(r = 0; r < (NRUNS+1); r++)
    				{
    					cublasSetMatrix((long)m, (long)n, sizeof(cuDoubleComplex), C, (long)LDC, dC_single, (long)ldc);
					...
					cublasGetMatrix((long)m, (long)n, sizeof(cuDoubleComplex), dC_single, (long)ldc, csingle, (long)LDC);
			...
			// mgpu test
			{
				...
				{
    				// make a copy of C
    				memcpy(cmgpu, C, size_c * sizeof(cuDoubleComplex));
    		...
    		// testing correctness
    		{
    			if(alloc == 1 && alloc_mgpu == 1)
    			{
    				// testing error -- specify ref. vector and result vector
      				cuDoubleComplex* cref = csingle; 
      				cuDoubleComplex* cres = cmgpu;
