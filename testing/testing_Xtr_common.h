...
/**
 * @file testing/testing_Xtr_common.h
...
void cublasXaxpy (int n, hipComplex alpha, const hipComplex *x, int incx, hipComplex *y, int incy){
  hipblasCaxpy (n, alpha, x, incx, y, incy);
}
void cublasXaxpy (int n, hipDoubleComplex alpha, const hipDoubleComplex *x, int incx, hipDoubleComplex *y, int incy){
...
// float Xabs(hipFloatComplex a){return cget_magnitude(a);}
// double Xabs(hipDoubleComplex a){return zget_magnitude(a);}
...
void start_timing(hipStream_t curStream){
  check_error( hipEventRecord(start, curStream) );
  check_error( hipGetLastError() );
}
float get_elapsed_time(hipStream_t curStream){
  check_error( hipEventRecord(stop, curStream) );
  check_error( hipGetLastError() );
...
{                                                                 \
  hipError_t err = hipMalloc( (void**)&ptr, (size)*sizeof(T) ); \
  if(err != hipSuccess)                                          \
  {                                                               \
  ...
  // if(!_kblas_error( (err), __func__, __FILE__, __LINE__ ))     \
    // return 0;                                                  \
//  check_error( hipMalloc( (void**)&ptr, (size)*sizeof(T) ) )

#define TESTING_MALLOC_PIN( ptr, T, size)                       \
  check_error( hipHostMalloc ( (void**)&ptr, (size)*sizeof( T ), hipHostMallocPortable  ))
